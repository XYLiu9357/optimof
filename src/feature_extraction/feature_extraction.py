...
Optimized file I/O pipeline. Original design by Kulik's Group @MIT: 
...
def extract_features(project_path, target_path, prediction_type, id=""):
    """
    ...
    if (f"temp_{id}") in os.listdir(project_path):
        temp_found_warning = "WARNING: there is another feature extraction in progress. Overwrite? [y/n] "
        while True: 
    ...
    RAC_finder_path = os.path.join(project_path, "src", "feature_extraction", "RAC_finder.py")
    ...
    dict_list = []
    cif_file = target_path + "_primitive.cif"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1: 
        print("No target given. Running feature extraction on ABAVIJ_clean.cif")
        target = os.path.join(project_path, "test", "ABAVIJ_clean.cif")
    else: 
        target = os.path.join(project_path, sys.argv[1])

    # Generate descriptors 
    result = extract_features(project_path, target, "thermal")
    if result.empty: 
    ...
    print(result)
    print("***Descriptor generator exited successfully***")
