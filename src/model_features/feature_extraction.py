...
def extract_features(project_path, target_path, id=""):
    ...
    temp_dir = project_path / f"temp_{id}"
    ...
    cif_dir = temp_dir / "cifs"
    cif_copy_path = cif_dir / "temp.cif"
    ...
    RAC_dir = feature_dir / "RAC"
    zeo_dir = feature_dir / "zeo++"
    os.mkdir(feature_dir)
    os.mkdir(RAC_dir)
    os.mkdir(zeo_dir)

    # Generate features
    cif_primitive_path = cif_dir / "temp_primitive.cif"
    shutil.copy(cif_copy_path, cif_primitive_path)

    # Define path variables for clarity
    zeo_path = project_path / "zeo++-0.3" / "network"
    RAC_finder_path = project_path / "src" / "model_features" / "RAC_finder.py"

    pd_path = zeo_dir / "temp_pd.txt"
    sa_path = zeo_dir / "temp_sa.txt"
    pov_path = zeo_dir / "temp_pov.txt"

    # cmd1, cmd2, and cmd3 are for Zeo++. cmd4 is for RACs.
    cmd1 = str(zeo_path) + " -ha -res " + str(pd_path) + " " + str(cif_primitive_path)
    cmd2 = str(zeo_path) + " -sa 1.86 1.86 10000 " + str(sa_path) + " " + str(cif_primitive_path)
    cmd3 = str(zeo_path) + " -volpo 1.86 1.86 10000 " + str(pov_path) + " " + str(cif_primitive_path)
    cmd4 = "python " + str(RAC_finder_path) + " %s %s" % (str(cif_primitive_path), str(RAC_dir))
    ...
    dict_list = []
    cif_file = str(cif_copy_path) + "_primitive.cif"
    ...
    if (
        os.path.exists(zeo_dir / "temp_pd.txt")
        & os.path.exists(zeo_dir / "temp_sa.txt")
        & os.path.exists(zeo_dir / "temp_pov.txt")
    ):
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        target = "ABAVIJ_clean"
        target_path = project_path / "test" / "ABAVIJ_clean.cif"
