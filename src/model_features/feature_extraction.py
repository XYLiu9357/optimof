...
def extract_features(project_path, target_path, id=""):
    ...
    RAC_finder_path = os.path.join(
        project_path, "src", "model_features", "RAC_finder.py"
    )
    ...
    cmd1 = zeo_path + " -ha -res " + pd_path + " " + cif_primitive_path
    cmd2 = zeo_path + " -sa 1.86 1.86 10000 " + sa_path + " " + cif_primitive_path
    cmd3 = zeo_path + " -volpo 1.86 1.86 10000 " + pov_path + " " + cif_primitive_path
    cmd4 = "python " + RAC_finder_path + " %s %s" % (cif_primitive_path, RAC_dir)
    ...
    process = subprocess.Popen(cmd1, stdout=subprocess.PIPE, stderr=None, shell=True)
    output1 = process.communicate()[0]

    process = subprocess.Popen(cmd2, stdout=subprocess.PIPE, stderr=None, shell=True)
    output = process.communicate()[0]

    process = subprocess.Popen(cmd3, stdout=subprocess.PIPE, stderr=None, shell=True)
    output3 = process.communicate()[0]

    process = subprocess.Popen(cmd4, stdout=subprocess.PIPE, stderr=None, shell=True)
    ...
    # Take mean
    lc_df_numeric_mean = (
        lc_df.select_dtypes(include=[np.number]).mean().to_frame().transpose()
    )
    sbu_df_numeric_mean = (
        sbu_df.select_dtypes(include=[np.number]).mean().to_frame().transpose()
    )
    linker_df_numeric_mean = (
        linker_df.select_dtypes(include=[np.number]).mean().to_frame().transpose()
    )

    merged_df = pd.concat(
        [geo_df, lc_df_numeric_mean, sbu_df_numeric_mean, linker_df_numeric_mean],
        axis=1,
    )
