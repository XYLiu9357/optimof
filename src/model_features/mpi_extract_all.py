...
def extract_all(project_path, src_dir, dest_dir, max_lim=20):
    ...
    if rank == 0:
        all_feature_df = pd.concat([df for sublist in gathered_results for df in sublist])
        return all_feature_df


def extract_all_to_csv(project_path, src_dir, dest_dir, file_name="features.csv", max_lim=20) -> None: 
    """
